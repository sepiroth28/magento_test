...
class Shopshopnshop_Tntshipping_Adminhtml_TntshippingdescriptionController extends Mage_Adminhtml_Controller_action
{
...
//		$this->_forward('edit');
	...
	public function editAction() {
		...
		$model  = Mage::getModel('tntshipping/tntshippingdescription')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('tntshipping/adminhtml_tntshippingdescription_edit'))
				->_addLeft($this->getLayout()->createBlock('tntshipping/adminhtml_tntshippingdescription_edit_tabs'));
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			$content = Mage::helper('tntshipping/readcsv')->readCSV($csv_file);
					...
					try {
								...
								$model = Mage::getModel('tntshipping/tntshippingdescription');
								$model->setData($data)
        ...
        if(!is_array($tntshippingIds)) {
            ...
            try {
                foreach ($tntshippingIds as $tntshippingId) {
                    $tntshipping = Mage::getModel('tntshipping/tntshippingdescription')->load($tntshippingId);
                    $tntshipping->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($tntshippingIds)) {
            ...
            try {
                foreach ($tntshippingIds as $tntshippingId) {
                    $tntshipping = Mage::getSingleton('tntshipping/tntshippingdescription')
                        ->load($tntshippingId)
